            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <ul class="nav nav-tabs">
                        ...
                        @foreach(RamdanRiawan\Table::getEnum(new \App\Models\BreakTime(), 'break_type') as $item)
                            <li>
                                <a href="{{ url(route('web.admin.breakTime.index', ['date' => request('date'), 'break_type' => $item])) }}"
                                   class="btn btn-xs  {{ request('break_type') == $item ? 'btn-primary text-white': 'btn-default' }}"
                                   style="{{ request('break_type') == $item ? 'color: white !important;': '' }} margin-right: 10px; margin-left: 10px;  display: inline-block;">{{ $item }}</a>
                            </li>
                        @endforeach
...
@section('script-in-this-page')
    <script>
        function redirectToBreakTimeByDate(date) {
            const baseUrl = "{{ url(route('web.admin.breakTime.index')) }}";
        ...
        $('#date-select').on('change', function () {
            ...
            redirectToBreakTimeByDate(selectedDate);
        });
    </script>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            let datatable1 = $('#dataTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{{ url(route('api.v1.web.admin.breakTime.dataTable')) }}',
                    data: {
